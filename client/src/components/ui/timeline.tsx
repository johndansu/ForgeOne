...
// ForgeOne specific work entry timeline
export function WorkEntryTimeline({ entries }: { entries: any[] }) {
  const timelineItems: TimelineItem[] = entries.map((entry) => ({
    id: entry.id,
    title: entry.title || "Untitled Entry",
    description: entry.description || `${entry.category} work - ${entry.timeSpent || 0} hours`,
    timestamp: entry.createdAt,
    status: entry.state_after === "Resolved" ? "completed" : 
             entry.state_after === "Advanced" ? "active" : "pending",
    icon: entry.category === "project" ? <Briefcase className="h-3 w-3" /> :
          entry.category === "study" ? <GraduationCap className="h-3 w-3" /> :
          entry.category === "personal" ? <Heart className="h-3 w-3" /> :
